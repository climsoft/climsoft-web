...
import {
    ...
    BulkRestoreExecuteDto,
    BulkRestoreExecuteResponse,
    BulkRestoreFilterDto,
} from '../dtos/bulk-restore.dto';
...
@Injectable()
export class BulkRestoreService implements OnModuleDestroy {
    private readonly logger = new Logger(BulkRestoreService.name);
    private readonly sessions: Map<string, BulkRestoreSession> = new Map();
    ...
    public async cleanupStaleSessions() {
        ...
        for (const [sessionId, session] of this.sessions) {
            if (now - session.lastAccessedAt > this.SESSION_TTL_MS) {
                this.logger.log(`Cleaning up stale bulk restore session: ${sessionId}`);
                await this.destroySession(sessionId);
            }
        }
    }

    public async checkForRestore(dto: BulkRestoreCheckDto): Promise<BulkRestoreCheckResponse> {
        ...
        if (totalMatchingRows === 0) {
            const session: BulkRestoreSession = {
        ...
        const csvFileName = `bulk_restore_${sessionId}.csv`;
        ...
        const tmpTable = `bulk_restore_tmp_${sessionId.replaceAll('-', '_')}`;
        ...
        const duckDbTableName = `bulk_restore_${sessionId.replaceAll('-', '_')}`;
        ...
        const previewData: BulkRestoreCheckResponse['previewData'] = { columns, rows, totalRowCount: totalMatchingRows };
    ...
    public async executeRestore(dto: BulkRestoreExecuteDto, userId: number): Promise<BulkRestoreExecuteResponse> {
        ...
        try {
            const updateSql = this.buildRestoreQuery(session.filter, userId);
            const updateResult = await queryRunner.query(updateSql.sql, updateSql.params);
            const restoredCount: number = updateResult[1] ?? 0;

            await queryRunner.commitTransaction();

            this.eventEmitter.emit('observations.restored');
    ...
    private getSession(sessionId: string): BulkRestoreSession {
        const session = this.sessions.get(sessionId);
        if (!session) {
    ...
    private buildCountQuery(filter: BulkRestoreFilterDto): { sql: string; params: any[] } {
    ...
    private buildPreviewSelectQuery(filter: BulkRestoreFilterDto): { sql: string; params: any[] } {
        const whereClause = this.buildFilterWhereClause(filter, 'o', 1);
    ...
    private buildRestoreQuery(filter: BulkRestoreFilterDto, userId: number): { sql: string; params: any[] } {
        const whereClause = this.buildFilterWhereClause(filter, 'observations', 1);
        const sql = `UPDATE observations SET deleted = false, entry_user_id = ${userId} WHERE deleted = true ${whereClause.sql}`;
        return { sql, params: whereClause.params };
    }

    private buildFilterWhereClause(filter: BulkRestoreFilterDto, tableAlias: string, startParamIndex: number): { sql: string; params: any[] } {
