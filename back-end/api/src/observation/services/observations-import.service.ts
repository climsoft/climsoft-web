    ...
    public async importProcessedFilesToDatabase(processedFiles: string[]): Promise<void> {
        for (const filePathName of processedFiles) {
            try {
                ...
                // The CSV file has columns: station_id, element_id, level, date_time, interval, source_id, value, flag, comment, entry_user_id
                // Note: entry_datetime is excluded as it's auto-generated by database trigger
                const copyQuery = `
                            COPY observations (${this.STATION_ID_PROPERTY_NAME}, ${this.ELEMENT_ID_PROPERTY_NAME}, ${this.LEVEL_PROPERTY_NAME}, ${this.DATE_TIME_PROPERTY_NAME}, ${this.INTERVAL_PROPERTY_NAME}, ${this.SOURCE_ID_PROPERTY_NAME}, ${this.VALUE_PROPERTY_NAME}, ${this.FLAG_PROPERTY_NAME}, ${this.COMMENT_PROPERTY_NAME}, ${this.ENTRY_USER_ID_PROPERTY_NAME})
                            FROM '${dbFilePathName}'
